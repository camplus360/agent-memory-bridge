...
One adapter per agent. Every adapter **only captures conversation events and sends them to the unified `claude-mem-worker.py` / worker**; no memory logic is duplicated. Summarization, embeddings and search all live in the claude-mem worker (`127.0.0.1:37701`).
